...
async function startQuizFromHelpMenu(interaction, topic) {
  // Acknowledge and defer - we need a deferred reply for the quiz command to editReply
  try {
    await interaction.deferReply();
  } catch (e) {
    // If deferReply fails, try deferUpdate as fallback
    try { await interaction.deferUpdate(); } catch (_) {}
  }
  ...
  if (!quizCommand) {
    return interaction.editReply({ content: '❌ Quiz command not found' });
  }
  ...
  try {
    const fakeInteraction = {
      ...
      replied: true,
      deferred: true,
      // Pass through to real interaction methods - already deferred
      deferReply: async () => {},
    ...
    try {
      await interaction.editReply({ content: `❌ Quiz generation failed. Use \`/quiz topic:${topic}\` directly!` });
    } catch (e) {
...
async function startLearnFromHelpMenu(interaction, topic) {
  // Acknowledge and defer - we need a deferred reply for the learn command to editReply
  try {
    await interaction.deferReply();
  } catch (e) {
    // If deferReply fails, try deferUpdate as fallback
  ...
  if (!learnCommand) {
    return interaction.editReply({ content: '❌ Learn command not found' });
  }
  ...
  try {
    const fakeInteraction = {
      ...
      commandName: 'learn',
      options: {
        getString: (name) => name === 'topic' ? actualTopic : (name === 'difficulty' ? 'beginner' : null),
        getInteger: () => null,
        getUser: () => null,
        getSubcommand: () => null,
        get: () => null
    ...
    try {
      await interaction.editReply({ content: `❌ Lesson generation failed. Use \`/learn topic:${actualTopic}\` directly!` });
    } catch (e) {
      // Silently fail if we can't edit
    }
  }
...
async function handleSovereignSyllabus(interaction) {
  ...
  const syllabusEmbed = new EmbedBuilder()
    ...
    .setDescription(`### ${typeIcons[recommendation.type]} ${typeLabels[recommendation.type]}\n**Suggested Lesson:** ${recommendation.topic.name}\n**Difficulty:** ${recommendation.topic.difficulty} • **Ref:** \`[${recommendation.topic.id}]\`\n\n**🧠 AI Reasoning:** ${recommendation.xaiReasoning}`)
    .addFields(
      { name: '📊 Progress', value: `\`[${progressBar(progressPercent)}]\` **${progressPercent}%** (${userXP}/${xpToNextLevel} XP)\n📈 **${xpRemaining} XP** to Level ${userLevel + 1} • Lesson: **+${projectedXP} XP**`, inline: false },
      { name: '💰 Multiplier HUD', value: `🔥 Streak: **${streakMultiplier.toFixed(2)}x** (${userStreak}d) • ✨ Prestige: **${prestigeMultiplier.toFixed(2)}x** (P${userPrestige})\n💎 **Total: ${totalMultiplier.toFixed(2)}x** → ${projectedXP} XP/lesson`, inline: false },
      { name: '📚 Your Stats', value: `📖 **${lessonsCompleted}** lessons • 🎯 **${quizzesTaken}** quizzes`, inline: true }
    )
